...
export class NaturalLanguageProcessor {
  ...
  async getEntity(value: string): Promise<any> {
    try {
      ...
      if (entity.entities && Array.isArray(entity.entities) && entity.entities.length > 0) {
        if (entity.intent === 'hotel') {
          return { type: 'hotel', name: entity.entities[0].value };
        } else if (entity.intent === 'country') {
          return {
            type: 'country',
            country: entity.entities.find(e => e.entity === 'country')?.value || null,
            city: entity.entities.find(e => e.entity === 'city')?.value || null
          };
  ...
  async initialize() {
    ...
    this.manager.addNamedEntityText('pais', ['en %country%', '%country%'], ['location']);
    this.manager.addNamedEntityText('ciudad', ['en %city%', '%city%'], ['location']);
    this.manager.addNamedEntityText('service', ['hotel con %service%'], ['service']);
    this.manager.addNamedEntityText('room', ['habitaciones %room%'], ['room']);


    this.manager.addDocument('es', 'hotel %name%', 'hotel');
    this.manager.addDocument('es', 'quiero reservar un hotel llamado %name%', 'hotel');
    ...
    (async() => {
      ...
      console.log('Response after traininggggggggggggggggggggggg:', testresponse);
