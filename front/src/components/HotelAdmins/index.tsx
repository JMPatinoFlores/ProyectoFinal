...
const HotelAdmins = ({ searchQuery }: IHotelAdminsProps) => {
  ...
  const [filteredHotelAdmins, setFilteredHotelAdmins] = useState<
    IHotelAdminDetails[]
  >([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedHotelAdmin, setSelectedHotelAdmin] =
    useState<IHotelAdminDetails | null>(null);
  const [selectedHotelAdminToSend, setSelectedHotelAdminToSend] =
    useState<IHotelAdminDetails | null>(null);
  ...
  const handleUpdateHotel = async () => {
    if (selectedHotelAdmin) {
      try {
        ...
        const response = await fetchUpdateHotelAdminDetails(
          selectedHotelAdmin.id,
          selectedHotelAdminToSend
        );
        if (response) {
          const updatedHotelAdmins = hotelAdmins.map((hotelAdmin) =>
            hotelAdmin.id === selectedHotelAdmin.id
              ? selectedHotelAdmin
              : hotelAdmin
          );
  ...
  const handleInputChange = (
    ...
    if (selectedHotelAdmin) {
      ...
      if (name === "birthDate") {
        const validDate =
          Number(value.split("-")[0]) > 1900 &&
          Number(value.split("-")[0]) < 2025;
        if (validDate) {
              ...
              <div className="bg-white p-4 rounded-lg shadow-lg w-full max-w-md">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col mb-4">
                    ...
                    {emailError && (
                      <small className="text-end text-base text-red-500">
                        {emailError}
                      </small>
                    )}
                  </div>
